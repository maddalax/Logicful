...
<div
  ...
  {#if config.prefix}
    <div class="bd-highlight pr-2 flex-shrink-1" style="width: 20%">
      <LiveField
        field={{ required: true, name: `${field.name}.prefix`, id: `${field.id}.value.prefix`, helperText: 'Prefix', hideLabel: true, value: value?.prefix?.value ?? '', type: 'string' }} />
    </div>
  {/if}
  {#if config.first}
    <div class="bd-highlight flex-grow-1 pr-2">
      <LiveField
        field={{ required: true, name: `${field.name}.first`, id: `${field.id}.value.first`, helperText: 'First Name', hideLabel: true, value: value?.first?.value ?? '', type: 'string' }} />
    </div>
  {/if}
  {#if config.middle}
    <div class="bd-highlight flex-grow-1 pr-2">
  ...
  {#if config.last}
    <div class="bd-highlight flex-grow-1 pr-2">
      <LiveField
        field={{ required: true, name: `${field.name}.last`, id: `${field.id}.value.last`, helperText: 'Last Name', hideLabel: true, value: value?.last?.value ?? '', type: 'string' }} />
    </div>
  {/if}
  {#if config.suffix}
    <div class="bd-highlight p-right-2 flex-shrink-1" style="width: 20%">
      <LiveField
        field={{ required: true, name: `${field.name}.suffix`, id: `${field.id}.value.suffix`, helperText: 'Suffix', hideLabel: true, value: value?.suffix?.value ?? '', type: 'string' }} />
    </div>
  {/if}
</div>
