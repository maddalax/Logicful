<script lang="typescript">
  ...
  import FormEdit from '@app/features/form/edit/FormEdit.svelte'
  ...
  <div class="col-12 mb-4">
    <div class="d-flex justify-content-end" style="padding-bottom: 1em; padding-left: 0; display: flex; text-align: right;">
      ...
      <div style="text-align: right; padding-left: 0.5em;">
        <a href={`/form/builder?formId=${form.id}`} target="_blank" class="btn btn-outline-dark"> <span class="fas fa-pencil-alt" /> Edit Form </a>
      </div>
    ...
    <form action="#" method="post" class="card border-light p-3 mb-4">
      {#if selected === 'general'}
        ...
        <div class="card-body p-0 p-md-4" style="padding-top: 0.5em !important;">
      ...
      {/if}
      <div class="d-flex justify-content-end ml-auto" style="padding-right: 1em; padding-bottom: 1em;">
        {#if saving}<button class="btn btn-primary" type="button" disabled>Saving...</button>{:else}<button class="btn btn-primary" type="button" on:click={saveDraft}>Save Changes</button>{/if}
      </div>
    </form>
  </div>
</div>
