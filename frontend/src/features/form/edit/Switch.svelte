...
<div class="form-check form-switch">
    <input class="form-check-input" type="checkbox" id={`${field.id}`} checked={value} on:input={(e) => {
      field.value = e.target.checked;
      dispatchFieldChange(field, true);
      field.onChange?.(field.value);
    }}>
    <label class="form-check-label" for={`${field.id}`}>{firstNotEmpty(field.label, field.name)}</label>
</div>